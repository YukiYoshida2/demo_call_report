---
name: fetch-data
description: Databricks MCP経由でQ1〜Q6のCSVデータを取得する。小規模(Q1-Q3)は直接取得、大規模(Q4-Q6)はチャンク分割で取得。フォアグラウンドAgent並列取得に対応。
---
...
## Agent並列化パターン（フォアグラウンド）

大規模データ取得時は**フォアグラウンドAgent × 4並列**で取得する。
Agent Team（TeamCreate + run_in_background）は使わない — Team Leadのターンが終了し、Agent完了後に自動再開しないため。

```
メイン → Agent A: Q1 + Q2 + Q3 を取得（小規模、即完了）
       → Agent B: Q4 取得（~20,000行、チャンク分割）
       → Agent C: Q5 取得（数千行、チャンク分割）
       → Agent D: Q6 取得（~13,000行、チャンク分割）
（4つのAgentツール呼び出しを1メッセージで同時発行。全Agent完了までメインはブロッキング待機）
```

**重要ルール:**
- `team_name` パラメータは使わない
- `run_in_background: true` は使わない（フォアグラウンドで実行）
- TeamCreate / TeamDelete は使わない
- SendMessage は使わない（Agent結果はツール返却で直接受け取る）
- 各Agentは完了時に「CSV行数・エラー有無・保存先パス」を含む簡潔なサマリを返すこと

各Agentは `data/{YYYY-MM-DD}/` ディレクトリにCSVを保存する。4 Agentのツール結果が全て返ってきた時点で、メインがStep 3: データ検証に進む。

### 結果判定（メインAgent）

4 Agentのツール結果を受信後、以下の判定を行う:

| 成功Agent数 | 対応 |
|------------|------|
| 4/4 | 正常。Step 3: データ検証に進む |
| 2-3/4 | 成功分のCSVは有効。失敗分をユーザーに報告し、単体リトライを提案 |
| 0-1/4 | システム障害の可能性。ユーザーに報告して停止 |
